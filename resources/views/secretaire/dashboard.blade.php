    ...
    <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
        <div class="mx-auto max-w-screen-xl space-y-6 px-4 lg:px-8">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div class="rounded-xl border border-teal-200 bg-gradient-to-br from-teal-900 to-teal-800 p-6 text-white shadow-lg dark:border-teal-800">
                    <p class="text-sm font-medium text-teal-100">Dossiers transmis vers un autre service</p>
                    <p class="mt-1 text-4xl font-bold tracking-tight">{{ $transferredTotal }}</p>
                    <p class="mt-2 text-sm text-teal-200">Au moins une retransmission enregistrée (trace conservée).</p>
                    @if($transferredTotal > 0)
                        <a href="{{ route('secretaire.forms.index', ['transferred' => 1]) }}" class="mt-3 inline-flex items-center rounded-lg bg-white/10 px-3 py-1.5 text-sm font-medium text-white hover:bg-white/20">
            ...
            <div class="space-y-6"
                 x-data="secretaireDossierPollBatch({ pollUrl: '{{ route('secretaire.forms.poll') }}', ids: @json($pollDashboardIds) })">
            ...
            {{-- Dossiers ayant été transmis (trace inter-services) --}}
            <div class="overflow-hidden rounded-lg border border-teal-200 bg-white shadow-sm dark:border-teal-900 dark:bg-gray-800">
                <div class="border-b border-teal-100 px-4 py-3 dark:border-teal-900/60">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-white">Dossiers transmis</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Récemment retransmis vers un autre service — la référence d’origine est conservée sur la fiche.</p>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-teal-50/80 dark:bg-gray-900/50">
                            <tr>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Référence</th>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Dernier transfert</th>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Transferts</th>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Objet</th>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Statut</th>
                                <th scope="col" class="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400"></th>
                            </tr>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($transferredRecent as $f)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <td class="whitespace-nowrap px-4 py-3 text-sm font-medium text-gray-900 dark:text-white">{{ $f->reference }}</td>
                                    <td class="whitespace-nowrap px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $f->last_transferred_at?->format('d/m/Y H:i') ?? '—' }}</td>
                                    <td class="whitespace-nowrap px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $f->transfers_count }}</td>
                                    <td class="max-w-xs truncate px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ \Illuminate\Support\Str::limit($f->objet, 48) }}</td>
                                    <td class="whitespace-nowrap px-4 py-3 text-sm">
                                        <span data-secretairepoll-status="{{ $f->id }}"
                                            class="inline-flex px-2.5 py-0.5 rounded text-xs font-medium
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3 text-right text-sm">
                                        <a href="{{ route('secretaire.forms.show', $f) }}" class="font-medium text-teal-800 hover:text-teal-950 dark:text-teal-400 dark:hover:text-teal-300">Ouvrir</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Aucun dossier transmis pour ce service.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                @if($transferredTotal > 15)
                    <div class="border-t border-gray-200 px-4 py-3 text-right dark:border-gray-700">
                        <a href="{{ route('secretaire.forms.index', ['transferred' => 1]) }}" class="text-sm font-medium text-teal-800 hover:underline dark:text-teal-400">Voir tous les dossiers transmis ({{ $transferredTotal }})</a>
                    </div>
                @endif
            </div>
